...
def init_db():
    with get_db() as conn:
        ...
        # Store Locations (for multi-branch support)
        conn.execute('''
        ...
        # Users Table (Updated with store_id)
        conn.execute('''
        ...
        conn.execute('''
            INSERT OR IGNORE INTO subscription_plans (plan_name, plan_code, description, price, billing_cycle, max_users, max_locations, max_transactions, features)
            VALUES 
        ...
        conn.execute('''
            INSERT OR IGNORE INTO super_admins (username, password, email, full_name, role)
...
def reset_password():
    ...
    try:
        ...
        with get_db() as conn:
            ...
            result = conn.execute('UPDATE users SET password = ? WHERE username = ? AND store_id = ?',
        ...
        with get_db() as conn:
            ...
            if len(str(code)) >= 4:
                conn.execute('''
                    ...
                    SET verified = 1, is_enabled = 1, updated_at = CURRENT_TIMESTAMP
    ...
    try:
        ...
        with get_db() as conn:
            ...
            conn.execute('''
                ...
                VALUES (?, 1, 0, 'trial', 'completed', datetime('now'), datetime('now', '+30 days'))
            ''', (store_id,))
            
            # Update store subscription
            conn.execute('''
                UPDATE stores 
                SET subscription_plan = 'free_trial', subscription_status = 'active', trial_ends_at = datetime('now', '+30 days')
                WHERE id = ?
            ''', (store_id,))
